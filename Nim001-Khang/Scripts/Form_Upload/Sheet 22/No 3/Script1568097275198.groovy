...
for (int i = 2; i <= updatekubun; i++) {
    ...
    def recordset1 = CustomKeywords.'com.database.connectSql.executeQuery'(('update IMFR_UT_SF_NIM001_APL002_TB1 set  IMFR_UD_TB1_UPDATE_KUBUN = \'' + 
        i) + '\' ,IMFR_UD_TB1_JYORYO_SYS_CODE = \'11111111117\' where IMFR_SD_INSERT_ID = \'8fbl8qi8zxe6tff\'')
    ...
    if (i == 1) {
        ...
        WebUI.takeScreenshot(('D:\\KatalonScreenShot\\Sheet22\\No3-' + i) + '.png' // Take screen shot
            )
		errMsg = WebUI.getText(findTestObject('Object Repository/Form_Upload/label_errMsg_Approve'))
        WebUI.verifyNotMatch(errMsg, '処理に失敗しました。 上流システムコードが取引停止になっています。差戻してください。', true, FailureHandling.STOP_ON_FAILURE)
    }
    ...
    if ((i == 2) || (i == 3)) {
        ...
        WebUI.verifyMatch(errMsg1, '処理に失敗しました。 上流システムコードが取引停止になっています。差戻してください。', true, FailureHandling.STOP_ON_FAILURE)
    }
    WebUI.delay(2)
