...
for (int i = 2; i <= updatekubun; i++) {
    ...
    def recordset1 = CustomKeywords.'com.database.connectSql.executeQuery'((((('update IMFR_UT_SF_NIM001_APL002_TB1 set  IMFR_UD_TB1_UPDATE_KUBUN = \'' + 
        i) + '\' ,IMFR_UD_TB1_JYORYO_SYS_CODE = \'11111111117\' where IMFR_SD_INSERT_ID =') + '\'') + GlobalVariable.insert_ID) + 
        '\'')
    ...
    WebUI.verifyMatch(errMsg1, '処理に失敗しました。 上流システムコードが取引停止になっています。差戻してください。', true, FailureHandling.CONTINUE_ON_FAILURE)
    WebUI.delay(2)
