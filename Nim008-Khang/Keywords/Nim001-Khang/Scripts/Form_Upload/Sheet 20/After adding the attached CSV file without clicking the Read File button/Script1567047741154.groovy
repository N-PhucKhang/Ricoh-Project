...
WebUI.verifyEqual(errMsg, '添付ファイルを追加後にファイル読み込みボタンを押していません。ファイル読み込みボタンを押してCSVファイルチェックを行ってください', FailureHandling.CONTINUE_ON_FAILURE)
WebUI.click(findTestObject('Object Repository/Form_Upload/button_ReadFile' // Click button ReadFile
        ))
WebUI.delay(2)
WebUI.click(findTestObject('Form_Upload/button_AddFile' // Click button Add File
        ))
WebUI.setText(findTestObject('Form_Upload/button_AddFile_Popup'), Folder_Url + GlobalVariable.NoError, FailureHandling.STOP_ON_FAILURE // Up file no_error.csv
    )
WebUI.delay(2)
WebUI.click(findTestObject('Form_Upload/button_UpFile_Popup' // Click button upload file in popup page
        ))
...
WebUI.verifyEqual(errMsg, '添付ファイルが編集されています。一括更新データ確認 を行ってください', FailureHandling.CONTINUE_ON_FAILURE // Verify error message
    )
