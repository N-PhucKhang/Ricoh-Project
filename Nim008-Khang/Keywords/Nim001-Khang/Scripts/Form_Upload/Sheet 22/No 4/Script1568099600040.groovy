...
def recordset1 = CustomKeywords.'com.database.connectSql.executeQuery'((('update IMFR_UT_SF_NIM001_APL002_TB1 set IMFR_UD_TB1_SHOHIN_CODE = \'A00111\' ,  IMFR_UD_TB1_UPDATE_KUBUN =  \'1\'  where IMFR_SD_INSERT_ID =' + 
    '\'') + GlobalVariable.insert_ID) + '\'')
...
WebUI.verifyMatch(errMsg, '処理に失敗しました。 商品コードに登録されていないコードが設定されています。差戻してください。', false, FailureHandling.CONTINUE_ON_FAILURE)
'Close browse'
WebUI.closeBrowser()
