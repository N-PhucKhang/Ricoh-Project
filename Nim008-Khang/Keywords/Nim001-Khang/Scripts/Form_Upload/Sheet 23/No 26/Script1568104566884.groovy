...
for (int i = 1; i <= totalFile; i++) {
    ...
    WebUI.verifyEqual(MsgErr1, '請求売上サイトID、商品コードが重複しています', FailureHandling.CONTINUE_ON_FAILURE)
    MsgErr2 = WebUI.getText(findTestObject('Form_Upload/check_Text2'))
    WebUI.verifyEqual(MsgErr2, '請求売上サイトID、商品コードが重複しています', FailureHandling.CONTINUE_ON_FAILURE)
