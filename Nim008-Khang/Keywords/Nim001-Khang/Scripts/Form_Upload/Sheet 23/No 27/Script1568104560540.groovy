...
for (int i = 1; i <= totalFile; i++) {
    ...
    WebUI.delay(2)
    WebUI.setText(findTestObject('Form_Upload/button_AddFile_Popup'), Folder_Url + GlobalVariable.No27 + 
    ...
    WebUI.verifyEqual(MsgErr1, '取引先コード、商品コードが重複しています', FailureHandling.CONTINUE_ON_FAILURE)
    MsgErr2 = WebUI.getText(findTestObject('Form_Upload/check_Text2'))
    WebUI.verifyEqual(MsgErr2, '取引先コード、商品コードが重複しています', FailureHandling.CONTINUE_ON_FAILURE)
